          ...
          {Object.entries(rest).map(([k,v]) =>(
              <div >
                <table className="w-full  border-0  px-4 py-2 text-left">
                  ...
                  <tbody>
                    <tr id ={k}>
                    <td className=" w-1/4 px-4 py-2 text-left">{k.toUpperCase()}</td>
                    <td className={`w-100  px-4 py-2 text-left ${k=="email"?"":"capitalize"}`}>{typeof v === "object"?JSON.stringify(v):v}</td>
                  </tr>
                  </tbody>
